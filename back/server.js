...
mongoose
  .connect(
    "mongodb+srv://anrds:1234@cluster0.iowtq.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
  )
  .then((res) => {
...
app.post("/patients", authenticateToken, async (req, res) => {
  try {
    ...
    const user_id = "U00" + (parseInt(patientCount) + 1);
    ...
    console.log(newPatient); // Log the new patient data for debugging
...
app.put("/patients/:id", authenticateToken, async (req, res) => {
  ...
  const patient = await Patient.findByIdAndUpdate(
    req.params.id,
    { email, name, age, gender, bloodGroup },
    { new: true }
  );
  res.send(patient);
});
